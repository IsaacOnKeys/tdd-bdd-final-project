...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_invalid_price_type(self):
        """It should raise a decimal.InvalidOperation when 'price' is not a valid decimal"""
        product = Product()
        data = {
            "name": "Hat",
            "description": "A fancy hat",
            "price": "twenty-five",  # Invalid price
            "available": True,
            "category": "CLOTHS"
        }
    ...
    def test_update_product_with_no_id(self):
        ...
        product = ProductFactory()
        product.id = None  # Ensure the ID is not set
        with self.assertRaises(DataValidationError) as context:
            product.update()
        self.assertEqual(
            str(context.exception), "Update called with empty ID field"
        )
